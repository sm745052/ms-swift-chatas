    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        for i in range(self.n_try_fetch):
            ...
            try:
                ...
                conv = unrolled_filter_min_one_image[
                    rng.randint(len(unrolled_filter_min_one_image))
                ]
                splits = conv.create_splits()
                random_split = splits[rng.randint(len(splits))]
                data = self.transform_dialog_data_to_raw_data(*random_split)
                return self.encode_func(data)
            except Exception:
                if i == self.n_try_fetch - 1:
                    if self.strict:
...
class PackingPreprocessor(EncodePreprocessor):
    ...
    def batched_preprocess(
        self, batched_row: Dict[str, Any], **kwargs
    ) -> Dict[str, Any]:
    ...
    def calculate_matched_group(self, sequences):
        ...
        sequences = binpacking.to_constant_volume(
            sequences, self.template.max_length, weight_pos=1
        )
        res = []
